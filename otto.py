...
SERVER_LINKS = { "support@support.whatsapp.com": None, "android@support.whatsapp.com": None, "smb@support.whatsapp.com": None, "press@whatsapp.com": None, "jan@whatsapp.com": None, "webmaster@whatsapp.com": None, "privacy@support.whatsapp.com": None, "abuse@support.whatsapp.com": None }

USER_SESSIONS = {}  # user_id -> {"server": email, "account": {email, password}}

def send_email(from_email, password, to_email, subject, body): msg = MIMEText(body) msg["Subject"] = subject msg["From"] = from_email msg["To"] = to_email

with smtplib.SMTP_SSL("smtp.gmail.com", 465) as server:
    server.login(from_email, password)
    server.send_message(msg)

async def start(update: Update, context: ContextTypes.DEFAULT_TYPE): user_id = update.effective_user.id if user_id != OWNER_ID: return
...
async def choose_server(update: Update, context: ContextTypes.DEFAULT_TYPE): user_id = update.effective_user.id if user_id != OWNER_ID: return
...
async def get_server(update: Update, context: ContextTypes.DEFAULT_TYPE): email_selected = update.message.text.strip() user_id = update.effective_user.id
...
async def get_subject(update: Update, context: ContextTypes.DEFAULT_TYPE): context.user_data["subject"] = update.message.text await update.message.reply_text("✉️ أرسل نص الرسالة:") return GET_BODY

async def get_body(update: Update, context: ContextTypes.DEFAULT_TYPE): user_id = update.effective_user.id session = USER_SESSIONS[user_id] server_email = session["server"] acc = session["account"] subject = context.user_data["subject"] body = update.message.text
...
async def check_replies(app): while True: for acc in ACCOUNTS: try: mail = imaplib.IMAP4_SSL("imap.gmail.com") mail.login(acc["email"], acc["password"]) mail.select("inbox") status, messages = mail.search(None, "UNSEEN") mail_ids = messages[0].split()
...
for mail_id in reversed(mail_ids):
                ...
                for server_email, user_id in SERVER_LINKS.items():
                    if user_id and server_email.lower() in from_.lower():
                        await app.bot.send_message(user_id, f"📨 رد من {server_email}:\n*{subject}*\n\n{body[:1000]}", parse_mode="Markdown")
                        SERVER_LINKS[server_email] = None
        except:
            pass
